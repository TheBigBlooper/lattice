        ...
        return connection.createDynamicReceiver().compose(receiver -> {
            ...
            return connection.createSender(MANAGEMENT_ADDRESS).compose(sender -> {
                sender.send(AmqpMessage.create()
                        ...
                        .build());
                // Both links are closed however the request ends. They are per-request - the reply
                // address is generated by the broker each time - so leaving them open leaks a sender
                // and a receiver on every poll, and the broker eventually starts rejecting the sends.
                return replied.future().eventually(() -> sender.close().compose(sent -> receiver.close()));
            });
        });
    }
