...
>
> **Which scenarios have crossed it.** Three depend on the boundary. **Peer expiry is proven across clusters** (a peer ages out to `UNREACHABLE` on TTL, retains its last-known detail, and does not change the observer's own verdict), and **revocation and foreign-authority refusal are proven across it too**: a revoked certificate is refused by the enforcing baseline after only THAT baseline's Secret is rewritten and its broker rolled - nothing in the revoked baseline's cluster is edited - and a well-formed certificate from an untrusted authority is refused outright.
>
> The rest - `degraded`, `baseline-down`, `mesh-cut`, `loop-check` - assert *local* behaviour that the boundary does not change: a service failing changes its own baseline's rollup, a broker outage changes its own mesh-link state, and loop prevention is a property of `max-hops` rather than of distance. They now run here too, not because the boundary tests them, but because compose was their only home and it is retired. Every scenario carries a control case, because "the handshake failed" passes just as loudly when the handshake was never attempted.

The local three-baseline stack (`deploy/k8s/mesh-clusters.sh`) therefore remains where the topology is exercised, and it is not a temporary stand-in. It is also now the **only** local stack: compose retired with locked #77, and every scenario it used to prove - including the three local ones and loop prevention - runs against the three kind clusters.
