...
| Mesh link         | A baseline's own connection to its own broker. Its state is served on that baseline's API only and never announced, so an operator can tell "we are cut off" from "the peers are gone". Keeps this name in the contract (`meshLink`); the console calls it **Broker**, because naming one of two links after the whole system is what made a healthy reading read as a healthy mesh (locked #80). | locked_decisions.md #46, #80; mesh_broker_topology.md   |
| Federation link   | One broker's connection to one **peer** broker, authenticated by certificate - distinct from the mesh link beneath it, and the thing a revoked or expired certificate actually breaks. Measured per peer from the broker, served on the owning baseline's API only, and never announced. The console calls it **Federation**. | locked_decisions.md #80; federation_link_visibility.md  |
