...
- **What is genuinely missing is identities, not scripting.** A headless token path already exists (`kc_token`, which every scenario assertion uses), so a script can hold a role today. What the realm does not have is any identity beyond the seeded `viewer` and `operator` - no third role, no per-person account, nothing to test a partial grant with. Adding one is a realm change and nothing else, and nothing currently needs it; the manual Authorize path covers the two that exist.
...
- All Elasticsearch access goes through the **Elasticsearch client + repositories** exported from `platform/lattice-common`. No direct connections or backdoor client imports from service code.
  - **Narrow exception - operational probes, not data access.** Reading a cluster's own `_cluster/health` to report it is monitoring, not data access: it reads no document, runs no query, and touches no mapping, so none of the reasons for this rule (scattered query logic, mapping drift, duplicated connection handling) apply. The mesh-gateway reads it over the shared `WebClient` it already uses to poll service readiness, because it is the one service with **no** Elasticsearch dependency at all - giving it a client purely to probe would add the dependency this rule's own single-writer reasoning exists to keep scarce. This exception covers **operational endpoints only** (`_cluster/health` and the like). The moment a service wants a document or a query, this rule applies in full. Detail in `baseline_component_reporting.md`.
