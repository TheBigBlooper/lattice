...
4. **Founder local-QA.** The founder verifies locally: build the affected module(s) and run the service / cluster (`deploy/k8s/mesh-clusters.sh`); pass -> `qa-passed`; fail -> back to step 2 with notes.
...
- **Version impact** - one line naming the bump this ticket's scope implies (`MAJOR` / `MINOR` / `PATCH`) **and why**, per [deploy_protocol.md - What a bump means](deploy_protocol.md#what-a-bump-means-and-who-proposes-it). The test is what a customer must *do* to take the change, not how big it was: "MINOR - a new operation, nothing existing changes" or "MAJOR - `CLUSTER_SERVICES` is renamed, so every values file must change". A docs-only or refactor ticket says `PATCH`. Proposed here because the person writing the change is the one who knows whether a variable moved; the release then takes the highest bump proposed since the last one.
...
| .claude/skills/gen-external-api/                     | Survey a repo (this one or any local checkout) and generate a full External API document, evidence-traced; modeled on `docs/replication/external_api.md`.                                                                       | Update when the survey flow or the exemplar's structure changes. Version-controlled.                         |
| .claude/skills/gen-srd/                              | Survey a repo (this one or any local checkout) and generate a full System Requirements Document; modeled on `docs/replication/system_requirements.md`.                                                                          | Update when the survey flow or the exemplar's structure changes. Version-controlled.                         |
| .claude/skills/gen-sdd/                              | Survey a repo (this one or any local checkout) and generate a full Software Design Document; modeled on `docs/replication/software_design.md`.                                                                                  | Update when the survey flow or the exemplar's structure changes. Version-controlled.                         |
| .claude/skills/gen-vdd/                              | Generate a Version Description Document for one release of a repo: shipped inventory, delta since the prior version, upgrade notes. Per-release output; no standing document in the repo.                                       | Update when the release-inventory flow changes. Version-controlled.                                          |
...
| .claude/agents/platform.md                           | The `platform` role agent: Docker, K8s/Helm, the Artemis mesh, the local kind stack, deploy (`deploy/*`).                                                                                                                       | Update when the platform surface or its rules change. Version-controlled.                                    |
...
| docs/protocol/qa_protocol.md                         | Local QA workflow: build + run the three-cluster kind stack, per-PR checklist convention.                                                                                                                                       | Update when the QA workflow changes.                                                                         |
...
| docs/protocol/platform_protocol.md                   | Platform rules: base image(s), K8s/Helm manifests, the Artemis mesh (broker + discovery), the local kind stack, cross-cluster interop.                                                                                          | Update when a platform, mesh, or container convention changes.                                               |
...
| docs/tour/demo_runbook.md                            | The live counterpart to the tour: a narrated order through the seven failure scenarios on a warm stack, with what to watch, expected recovery times, and what each asserts rather than displays.                                | Update when a scenario is added, removed, or changes what it asserts, or when a timing measures differently. |
| docs/tour/build_it_yourself.md                       | The reconstruction guide: the build order with an acceptance check per stage, the pinned-version manifest, and the registers of artifacts that must be reproduced and code built without a design doc.                          | Update when the build order, a pinned version, or a register entry changes.                                  |
| docs/replication/replication_prompt.md               | The hand-to-an-LLM replication prompt: mission, ground rules, staged build order with acceptance checks, stack manifest, and the authored-artifact list. Self-contained by design; excludes the demo domain.                    | Update when a stage, ground rule, pin reason, or authored artifact changes.                                  |
| docs/replication/software_design.md                  | The Software Design Document: architecture, module decomposition, component designs, data design, cross-cutting rules, and the design rationale register. Self-contained by design.                                            | Update when a component's design, a mechanism, or a rationale-register entry changes.                        |
| docs/replication/system_requirements.md              | The System Requirements Document: numbered shall statements per area, each with a verification method. The binding checklist of the replication pack. Self-contained by design.                                                | Update when a requirement-bearing decision changes; numbers are stable, withdrawn rows stay.                 |
| docs/replication/external_api.md                     | The External API document: every external interface of a baseline - REST conventions and platform operations, probes, docs pages, metrics scrape, the mesh wire protocol, and the federation link. Self-contained by design.   | Update when the contract, an envelope, a probe shape, or the federation interface changes.                   |
| docs/replication/failure_scenarios.md                | The failure scenario specification: each acceptance scenario's control, fault, assertions, and restore, plus the harness rules and an implementation skeleton. Self-contained by design.                                       | Update when a scenario is added, removed, or changes what it asserts.                                        |
| docs/replication/builders_reference.md               | The builder's reference: annotated excerpts of the authored artifacts, the platform configuration reference, and the worked domain-slice example carrying the write-operation patterns. Self-contained by design.              | Update when an authored artifact's load-bearing shape, an environment variable, or a carried pattern changes.|
...
| docs/planning/sprints/                               | Per-sprint plan docs (one file per sprint) - the canonical sprint plan mirrored by the master ticket. Produced via the sprint-plan skill. **Does not exist yet**; the first sprint creates it.                                  | Add one file per sprint at sprint start; update its wrap checklist as the sprint closes.                     |
...
| docs/design/architecture/baseline_configuration.md   | One authored source for baseline config: compose retired, the chart restructured as an umbrella with per-service subcharts, the realm templated from the console URL.                                                           | Update when a configuration-authoring or local-stack decision changes.                                       |
| docs/design/architecture/observability.md            | The instrumentation half of observability: the metrics engine, the management-port endpoint and its protection, what each layer measures, cardinality discipline, and what is deliberately not instrumented.                    | Update when a metric, its labels, the endpoint, or the collection-stack default changes.                     |
...
| docs/design/features/federation_link_visibility.md   | Federation link visibility: the two links and why one being up says nothing about the other, per-peer measurement from the broker, the Broker/Federation naming, and how a refused or expiring certificate reaches an operator. | Update when the federation signal, its states, its rendering, or the certificate check changes.              |
...
| docs/design/ui/metrics_view.md                       | The Metrics view: the contract operation that carries metrics to a browser, the cards and the series disclosure, session-only history, and the annotate-not-block divergence.                                                   | Update when a metrics-view or `getMetrics` decision changes.                                                 |
...
| deploy/certs/                                        | `issue-certs.sh` - the certificate authority + per-baseline broker certificates. The tool a **customer** runs; generated material is never committed.                                                                           | Update when the certificate model or its issuance changes.                                                   |
| deploy/k8s/                                          | The Helm chart + `mesh-clusters.sh`, the only local stack. **Never hand-edit generated output.**                                                                                                                                | Update when a manifest, chart, or the local stack changes; regenerate generated output.                      |
| .github/workflows/ci.yml                             | GitHub Actions CI on every push to `dev` or a `lat-*` branch: the Maven reactor, the console's gate, and the five CI-only jobs (OSV, Semgrep, gitleaks, chart render, actionlint).                                              | Update when CI steps, a trigger, or the JDK version changes.                                                 |
| deploy/k8s/chart/                                    | Where every environment variable is declared, in the values of the component that reads it. Replaced `.env.example`, which nothing read.                                                                                        | Update whenever a new env var is introduced.                                                                 |
