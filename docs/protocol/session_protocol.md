...
1. **The lint rule a protocol names may be CI-only.** A protocol or skill can cite a lint/style rule (e.g. the status-console "no hardcoded colors" scan, or a Checkstyle/Spotless rule) that is **not** part of the local `./mvnw verify` toolchain - it runs only in CI. An agent told to "run lint and confirm no hardcoded colors" cannot execute a CI-only rule locally; verify tokens by inspection and let CI run that gate. (Mark the exact rule names TBD until the lint/coverage gates land.)
