...
**This repository is public, so GitHub Actions runs free on standard runners and there is no minutes budget to ration** (locked #74, correcting the cost premise of #28/#53/#68). CI is therefore the **authoritative full-reactor run**, and the local hook is the **fast authoring gate** rather than the primary one. GitHub Actions (`.github/workflows/ci.yml`) triggers on `pull_request` into **`dev`** and into **`main`**, on `push` to **`dev`** and to any **`lat-*`** feature branch, plus manual `workflow_dispatch`. A concurrency group keeps one run per ref, so a branch with an open pull request does not stack a run per event.
