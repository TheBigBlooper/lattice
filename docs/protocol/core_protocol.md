...
- **Config files follow the same rule**, and one carve-out: a file may open with a **header block of up to five lines** saying what the file is and pointing at its doc. Everything after the header is an ordinary comment under the three-line cap. Helm values, templates, Dockerfiles, workflows, shell scripts, and poms are all in scope.
