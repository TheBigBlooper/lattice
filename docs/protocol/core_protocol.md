...
- **Order of operations (critical):**
  1. Claude writes the code **test-first** - the TDD red/green loop runs **locally** (a CI round-trip is far too slow to author against) - then runs the **full local gate (`./mvnw verify`, whole reactor)**, enforced by the committed pre-push hook (`.githooks/pre-push`), and **pushes the branch and stops**. **When the change touches container/deploy surface (a Dockerfile, a K8s manifest, the base image, the mesh broker wiring) and the local Docker toolchain is available, Claude also brings the affected service(s) up under docker-compose as verification** - a unit run cannot see an image build / wiring / runtime failure (see [platform_protocol.md](platform_protocol.md)). GitHub CI does **not** run on a feature-branch push or on a `dev` PR; it runs only on the `dev` -> `main` promotion PR (below). See [CI triggers + QA-iteration discipline](#ci-triggers--qa-iteration-discipline) below.
  ...
  3. **Founder says it passes** -> Claude opens the PR into `dev` (label `qa-passed`, QA results in the PR body). A `dev` PR runs **no** GitHub CI - the local pre-push `./mvnw verify` + founder QA are its gate.
...
- **Full local verify before every push - `./mvnw verify` (whole reactor) plus the status console's `verify`, via the pre-push hook.** This is the real build gate: the push aborts if either fails. Because it is the gate, the Maven half runs the **whole reactor**, not just the affected module. The Docker daemon must be up (Testcontainers integration suites). The console's half (Biome, types, Vitest, Semgrep, knip) is a separate command because the console is not a Maven module - see [ui_protocol.md](ui_protocol.md#quality-gates-the-consoles-half-of-the-pre-push-run). `./mvnw verify` on its own does not cover the console.
...
**No PR before local QA - not even a draft.** Push the branch (the pre-push hook verifies it), hand off for local QA, and open the `dev` PR only once the founder approves. A `dev` PR runs no GitHub CI, so there is nothing to gain by opening one early - and it still jumps the founder's QA gate. Never open a draft PR to "get CI".

**Iterating on an open PR.** A `dev` PR triggers no CI, so fix pushes to it are free - just keep the pre-push hook green and batch for tidiness. The skip-ci token only matters on the **`main` promotion PR** (the one PR that does run CI): if the founder requests a change on an open promotion PR, put the bracketed skip-ci token on the iteration commits to avoid CI churn, then do a final push **without** it as the pre-merge gate and merge on green.
