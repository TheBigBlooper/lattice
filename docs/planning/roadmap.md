...
- **Interop.** <- you are here. Cluster-to-cluster discovery + communication over the mesh; interoperability across divergent Elasticsearch data models proven. Discovery, federation, loop prevention, and per-baseline broker identity are proven; the remaining half is the Shape A redirect and unified view (#12, #28, #56), which are blocked on mockups.
