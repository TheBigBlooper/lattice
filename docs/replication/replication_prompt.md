...
- **The name, and every identifier carrying it (the naming rule).** The original is named Lattice, and its name rides inside identifiers throughout this pack: the announce address (`lattice.mesh.announce`), the broker federation role (`lattice_federation`) and federation names (`lattice-mesh-<baseline>`), the realm clients (`lattice-console`, `lattice-docs`), the `LATTICE_*` environment variables, the shared module names (`lattice-contract`, `lattice-common`), the Java namespace (`io.lattice`), and the metric family prefix. Choose your own system name and substitute it for `lattice` in **all** of them, consistently. The pack keeps the original's spellings so every example stays concrete and runnable; none of them is load-bearing beyond one constraint - a name shared across a mesh (the announce address, the federation role) must be spelled identically by every baseline in that mesh. Cluster and region names, and the console's branding, are equally yours.
