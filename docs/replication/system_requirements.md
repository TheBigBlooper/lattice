...
| DEP-010 | The system shall pass a scripted set of failure scenarios against the running three-baseline stack, each opening with a control asserting the healthy pre-state, restoring what it broke, and turning a failed assertion into a non-zero exit status. The set shall cover at least: a stopped service degrading the rollup, broker loss reading as "cut off" rather than "peers gone", peer expiry and return, certificate revocation refused, a foreign authority refused, federation loop prevention measured at one hop, and datastore degradation reported. | D      |
