...
- **`main` is never "ahead", so a plain commit count is the signal.** Under fast-forward promotion (locked #71) `main` cannot hold anything `dev` does not, so the old "judge by tree, because the merge commits make `main` look ahead" caveat no longer applies - there are no merge commits. `git rev-list --count origin/main..origin/dev` is the whole answer: zero means in sync, and any count is unpromoted work. If `git rev-list --count origin/dev..origin/main` is ever non-zero, that is a defect rather than drift - something reached `main` that never went through `dev`. Real drift is `dev` sitting many commits / weeks ahead. The Status Check surfaces this each session.
- **Promotion is non-destructive + founder-only.** Advance `main` by **fast-forward only** - `git checkout main && git merge --ff-only dev && git push`. It creates no commit, so `main` can never hold anything `dev` does not (locked #71). If `--ff-only` refuses, something was authored on `main` that should not have been: fix that rather than reaching for a merge. **Never** force-push or `reset --hard` `main` (Git Safety).
