...
/**
 * The inventory service verticle. It extends {@link BaseVerticle} for config, the health/readiness
 * surface, and graceful shutdown, and contributes only what is inventory-specific: the
 * OpenAPI-validated {@code /api/v1/inventory} routes ({@link #configureRoutes(Router)}) and an
 * Elasticsearch readiness check ({@link #registerReadinessChecks(HealthChecks)}), so
 * {@code /readiness} is DOWN until Elasticsearch is reachable.
 *
 * <p>Startup loads the shared config, builds the Elasticsearch client and repository, fires the
 * one-time create-if-absent bootstrap of both indices (not awaited, so a not-ready Elasticsearch does
 * not block the server from binding - readiness gates traffic instead), then loads the v1 OpenAPI
 * contract before delegating to the base start. The build is sequenced so the client is available by
 * the time the base mounts routes and readiness checks.
 */
    ...
    public Future<?> start() {
        return LatticeConfig.load(vertx)
                .compose(cfg -> {
                    ...
                    var indexBootstrap = new RetryingGate(() -> repository.bootstrap(baselineId()));
                ...
                .onSuccess(started -> LOG.info("inventory service started on port {}", actualPort()));
    }
