...
/**
 * The thin bootstrap for the mesh-gateway service. As a verticle it simply deploys the
 * {@link MeshGatewayVerticle}; as a program its {@link #main(String[])} stands up a Vert.x instance
 * and deploys itself (the launch entry point the container image runs).
 */
public final class MainVerticle extends VerticleBase {
    ...
    public static void main(String[] args) {
        ...
        vertx.deployVerticle(new MainVerticle()).onFailure(err -> {
            LOG.error("mesh-gateway service failed to deploy", err);
            vertx.close();
        });
    }
    ...
    @Override
    public Future<?> start() {
        return vertx.deployVerticle(new MeshGatewayVerticle());
    }
}
